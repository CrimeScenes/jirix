...
-- Function to calculate velocity prediction
local function getVelocityPrediction(character, part, isInAir)
    local humanoidRootPart = character:FindFirstChild("HumanoidRootPart")
    if humanoidRootPart then
        local velocity = humanoidRootPart.AssemblyLinearVelocity
        local predictionTime = isInAir and shared.Forbidden.Silent.Prediction or shared.Forbidden.TriggerBot.Settings.Preds.PredictionMultiplier

        -- Predict position based on velocity and prediction time
        ...
        if humanoid and humanoid.Health > 0 and not isPlayerKnocked(TargetPlayer) then
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    local isInAir = not humanoid:GetState() == Enum.HumanoidStateType.Physics

                    if onScreen and isWithinViewEra(part.Position) then
                        local predictedPosition = getVelocityPrediction(TargetPlayer.Character, part, isInAir)
                        local predictedScreenPos, onScreen = Camera:WorldToViewportPoint(predictedPosition)
